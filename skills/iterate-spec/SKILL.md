---
name: iterate-spec
description: Iterates existing PRD, TRD, and BDD specs from spec feedback by running a focused grill-with-docs clarification loop, then regenerating the specs with to-spec. Use when the user mentions /iterate-spec, asks to improve or update existing specs from feedback, or provides review comments, annotations, PR comments, or critique about existing PRD/TRD/BDD specs.
---
...
Given current specs plus feedback, resolve the feedback through a focused grilling session, then update the specs.

This is a spec-feedback iteration skill, not an initial spec-review generator. If no feedback source exists, ask for one or ask whether the user wants a separate review pass before iterating.
...
1. Identify inputs
   ...
   - Find the feedback source: pasted notes, review comments, annotations, PR comments, issue comments, or the current conversation.
...
3. Normalize feedback
   ...
   - Produce a short working list of feedback themes, contradictions, missing decisions, and spec-quality gaps.
...
4. Reiterate the plan with grill-with-docs
   ...
   - Focus on unresolved feedback and spec-quality gaps; do not re-litigate settled decisions unless feedback contradicts them.
...
- Every actionable feedback theme is reflected in the specs, explicitly rejected, or explicitly deferred.
- The specs do not contradict `CONTEXT.md`, ADRs, or observed code.
...
- Files changed.
- Traceability: feedback theme → decision → spec location or reason not changed.
